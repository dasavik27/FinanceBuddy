...
BENCHMARKS = {
    ...
    "Nifty Smallcap 250":"BSE-SMLCAP.BO",
    "Nifty 500":         "^NSEI",
    "Nifty Bank":        "^NSEBANK",
    "Nifty IT":          "^CNXIT",
    "Nifty 10yr G-Sec":  "LICNETFGSC.NS",
    "Gold":              "GOLDBEES.NS",
    "S&P 500 (US)":      "^GSPC",
    "Nasdaq 100 (US)":   "^IXIC",
}
...
FUND_BENCH_BY_CAP = {
    ...
    "Small Cap":       ("BSE-SMLCAP.BO",      "Nifty SmallCap 250"),
    "Flexi/Multi Cap": ("^NSEI",              "Nifty 500"),
    "Index":           ("^NSEI",              "Nifty 50"),
    "Mixed":           ("^NSEI",              "Nifty 50"),
}
FUND_BENCH_BY_CAT = {
    ...
    "FOF":    ("^NSEI",   "Nifty 500"),
    "Debt":   ("LICNETFGSC.NS", "Nifty 10yr G-Sec"),
    "Liquid": ("LICNETFGSC.NS", "Nifty 10yr G-Sec"),
    "Other":  ("^NSEI",   "Nifty 50"),
}
    ...
    try:
        ...
        for _, row in df_t.iterrows():
            ...
            if mask.any():
                ...
                # Fallback to the earliest available price if txn is before data start
...
def compute_fund_overlap(f1: pd.Series, f2: pd.Series) -> float:
    """
    Heuristic-based portfolio overlap estimation.
    In a real production environment, this would use stock-level holdings data.
    """
    if f1["Fund"] == f2["Fund"]:
        return 100.0
    
    cat1, cat2 = f1["Category"], f2["Category"]
    amc1, amc2 = f1["AMC"], f2["AMC"]
    cap1, cap2 = f1["Cap Type"], f2["Cap Type"]
    
    # 1. Asset Class Mismatch
    equity_cats = ["Equity", "ELSS", "Index", "FOF"]
    is_f1_eq = any(c in cat1 for c in equity_cats)
    is_f2_eq = any(c in cat2 for c in equity_cats)
    
    if is_f1_eq != is_f2_eq:
        return 0.0 # Zero overlap between Debt and Equity
        
    # 2. Category Similarity
    if cat1 == cat2:
        base = 60.0
        if amc1 == amc2: base += 25.0  # Same AMC usually has 80-90% overlap in same category
        if cap1 == cap2: base += 5.0
        return min(95.0, base)
    
    # 3. Cross-Cap Overlap (Heuristics)
    overlap = 10.0
    caps = {cap1, cap2}
    if "Large Cap" in caps and "Flexi/Multi Cap" in caps: overlap = 45.0
    elif "Large Cap" in caps and "Mid Cap" in caps: overlap = 20.0
    elif "Mid Cap" in caps and "Small Cap" in caps: overlap = 15.0
    elif cap1 == cap2 and cap1 != "Mixed": overlap = 50.0
    
    # AMC Boost (Shared research team)
    if amc1 == amc2:
        overlap += 15.0
        
    return min(95.0, overlap)


def get_sector_profile(category: str, fund_name: str) -> dict:
    """
    Estimate sector exposure profile based on category and fund strategy.
    Returns a dict of {Sector: Weight%}.
    """
    n = fund_name.upper()
    
    # Sector Specific Funds
    if "BANK" in n or "FINANCIAL" in n:
        return {"Financials": 85, "Services": 10, "Others": 5}
    if "IT" in n or "TECH" in n or "DIGITAL" in n:
        return {"Technology": 80, "Services": 15, "Others": 5}
    if "PHARMA" in n or "HEALTH" in n:
        return {"Healthcare": 80, "Consumer": 10, "Others": 10}
    if "INFRA" in n or "ENERGY" in n:
        return {"Energy": 40, "Industrial": 40, "Materials": 15, "Others": 5}
    if "AUTO" in n:
        return {"Consumer": 70, "Industrial": 20, "Others": 10}
        
    # Diversified Equity Profiles
    if category == "Large Cap" or "NIFTY 50" in n:
        return {"Financials": 34, "Technology": 14, "Energy": 12, "Consumer": 11, "Healthcare": 6, "Others": 23}
    if category == "Mid Cap":
        return {"Financials": 18, "Industrial": 16, "Consumer": 15, "Healthcare": 10, "Technology": 8, "Others": 33}
    if category == "Small Cap":
        return {"Industrial": 22, "Consumer": 18, "Financials": 12, "Materials": 12, "Healthcare": 8, "Others": 28}
    if category == "ELSS" or category == "Flexi/Multi Cap":
        return {"Financials": 28, "Technology": 12, "Consumer": 12, "Energy": 8, "Industrial": 8, "Others": 32}
    
    # Non-Equity fallback
    if category in ["Debt", "Liquid"]:
        return {"Govt Bonds": 60, "Corporate": 30, "Cash": 10}
        
    return {"Diversified": 40, "Financials": 20, "Technology": 10, "Consumer": 10, "Others": 20}


def get_estimated_holdings(category: str, fund_name: str) -> list:
    """
    Project likely top institutional holdings based on category benchmarks.
    Returns a list of strings (Company Names).
    """
    n = fund_name.upper()
    
    # 1. Sectoral Picks
    if "BANK" in n or "FINANCIAL" in n:
        return ["HDFC Bank", "ICICI Bank", "Axis Bank", "SBI", "Kotak Bank"]
    if "IT" in n or "TECH" in n:
        return ["Infosys", "TCS", "HCL Tech", "Wipro", "Tech Mahindra"]
    if "PHARMA" in n or "HEALTH" in n:
        return ["Sun Pharma", "Cipla", "Dr. Reddy's", "Apollo Hospitals", "Divi's Lab"]
        
    # 2. Large Cap / Index Core (The "Heavyweights")
    large_cap_core = ["HDFC Bank", "Reliance Industries", "ICICI Bank", "Infosys", "ITC", "TCS", "L&T"]
    if category == "Large Cap" or "NIFTY 50" in n or "SENSEX" in n:
        return large_cap_core
        
    # 3. Mid Cap Favorites
    if category == "Mid Cap":
        return ["Trent", "Cummins India", "PFC", "REC", "Astral", "Supreme Ind.", "Coforge"]
        
    # 4. Small Cap Alpha Picks
    if category == "Small Cap":
        return ["Kirloskar Oil", "KEI Industries", "Equitas Small Fin.", "Karur Vysya", "CIE India"]
        
    # 5. Multi/Flexi Cap Blend
    if category == "Flexi/Multi Cap" or category == "ELSS":
        return ["HDFC Bank", "ICICI Bank", "Infosys", "Reliance Industries", "Axis Bank", "Avenue Supermarts"]
        
    return ["Reliance Industries", "HDFC Bank", "ICICI Bank"]


def calculate_redemption_impact(df_t: pd.DataFrame, fund_name: str, units_to_redeem: float, current_nav: float) -> dict:
    """
    Simulate redemption using FIFO (First-In-First-Out) logic.
    Returns details of gain, tax, and acquisition history.
    """
    if df_t.empty or units_to_redeem <= 0:
        return {}
        
    # 1. Filter and sort transactions for this fund
    fund_txns = df_t[df_t["Fund"] == fund_name].sort_values("Date")
    
    # 2. Build the FIFO Queue of Unsold Units
    # We need to process ALL past transactions to see what units are left
    fifo_queue = [] # List of {"date": Date, "units": float, "nav": float}
    
    for _, tx in fund_txns.iterrows():
        u = float(tx.get("Units", 0))
        if u > 0:
            # Purchase
            fifo_queue.append({"date": tx["Date"], "units": u, "nav": float(tx.get("NAV", 0))})
        elif u < 0:
            # Redemption - remove from head (FIFO)
            to_remove = abs(u)
            while to_remove > 0 and fifo_queue:
                if fifo_queue[0]["units"] <= to_remove:
                    to_remove -= fifo_queue[0]["units"]
                    fifo_queue.pop(0)
                else:
                    fifo_queue[0]["units"] -= to_remove
                    to_remove = 0
                    
    # 3. Simulate Redemption on the remaining queue
    remaining_units = sum(q["units"] for q in fifo_queue)
    if units_to_redeem > remaining_units:
        units_to_redeem = remaining_units # Cap at actual holdings
    ...
    units_left_to_sell = units_to_redeem
    now = pd.Timestamp(datetime.now())
    
    for q in fifo_queue:
        if units_left_to_sell <= 0: break
        
        sell_from_this_batch = min(q["units"], units_left_to_sell)
        cost = sell_from_this_batch * q["nav"]
        value = sell_from_this_batch * current_nav
        gain = value - cost
        
        # Determine if STCG or LTCG (1 year threshold for Equity)
        is_ltcg = (now - q["date"]).days > 365
        ...
        simulation_results.append({
            "Acquisition Date": q["date"],
            "Units Sold": sell_from_this_batch,
            "Buy NAV": q["nav"],
            "Cost": cost,
            "Value": value,
            "Gain": gain,
        ...
        total_cost += cost
        units_left_to_sell -= sell_from_this_batch
        
    total_value = units_to_redeem * current_nav
    
    # 4. Tax Calculation (Post-2024 Rules)
    # STCG: 20%
    # LTCG: 12.5% (assuming above 1.25L limit)
    stcg_tax = max(0, total_stcg_gain * 0.20)
    ltcg_tax = max(0, total_ltcg_gain * 0.125)
    
    return {
        "total_units": units_to_redeem,
        ...
        "net_proceeds": total_value - (stcg_tax + ltcg_tax),
        "details": pd.DataFrame(simulation_results)
    }
