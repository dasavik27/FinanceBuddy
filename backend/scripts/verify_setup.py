...
def warn(name, condition, details=""):
    """Soft check: prints ✓/! but never fails the run."""
    if condition:
...
def main():
    ...
    warn(
        "FINANCEBUDDY_ADMIN_EMAILS (recommended)",
        bool(os.getenv("FINANCEBUDDY_ADMIN_EMAILS") or os.getenv("ADMIN_EMAILS")),
    )
    warn(
        "SUPABASE_SERVICE_ROLE_KEY (recommended for invites)",
        bool(os.getenv("SUPABASE_SERVICE_ROLE_KEY") or os.getenv("SUPABASE_SERVICE_KEY")),
    )
    origins = (os.getenv("FINANCEBUDDY_ALLOWED_ORIGINS") or "").strip()
    warn(
        "FINANCEBUDDY_ALLOWED_ORIGINS",
        bool(origins),
        "unset → CORS allows localhost only (breaks production frontend)",
    )
    if origins and "localhost" in origins and "vercel" not in origins.lower() and "https://" not in origins:
        warn(
            "FINANCEBUDDY_ALLOWED_ORIGINS looks local-only",
            False,
            "add your production frontend origin before deploying",
        )
    ...
    try:
        ...
        with pool.connection() as conn:
            ...
            tables = [
                "users", "identities", "profiles", "sessions",
    ...
    print("\n" + ("=" * 50))
    if all_ok:
        print("✓ Required checks passed. Review ! warnings above for production.")
        return 0
