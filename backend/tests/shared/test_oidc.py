...
def test_oidc_principal_from_claims_metadata_paths():
    from shared.oidc import OidcJwtVerifier, _principal_from_claims

    p = _principal_from_claims(
        {"sub": "u1", "user_metadata": {"email": "meta@test.com", "name": " Meta "}},
        "https://issuer",
    )
    assert p.email == "meta@test.com"
    assert p.name == "Meta"

    p2 = _principal_from_claims(
        {"sub": "u2", "app_metadata": {"email": "app@test.com"}, "user_email": "fallback@test.com"},
        "https://issuer",
    )
    assert p2.email == "app@test.com"
    ...
    with patch.object(verifier._jwks, "get_signing_key_from_jwt", side_effect=Exception("bad")):
        assert verifier.verify("tok") is None

def test_oidc_missing_sub_and_user_email(monkeypatch):
    from shared.oidc import OidcJwtVerifier, _principal_from_claims

    assert _principal_from_claims({"email": "x@test.com"}, "iss") is None
    p = _principal_from_claims({"sub": "u1", "user_email": " ue@test.com "}, "iss")
    assert p.email == "ue@test.com"
    ...
    key = MagicMock()
    key.key = "secret"
    monkeypatch.setattr(verifier._jwks, "get_signing_key_from_jwt", lambda t: key)
    monkeypatch.setattr(
        "jwt.decode",
        lambda *a, **k: {"sub": "", "iss": "https://issuer", "aud": "aud", "exp": 9999999999},
    )
    assert verifier.verify("tok.with.claims") is None
