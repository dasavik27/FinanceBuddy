...
def test_clear_benchmark_cache_from_fallbacks_suite():
...
def test_fetch_live_market_summary_fallback_when_yahoo_fails():
    """From test_fallbacks_and_indices_full — safe default summary."""
    market_indices.clear_benchmark_cache()
...
def test_market_indices_splice_empty():
    from shared.services import market_indices

    empty = pd.Series(dtype=float)
    assert market_indices._splice_benchmark_series(empty, empty).empty

def test_market_indices_uncached_and_hybrid_empty(monkeypatch):
    from shared.services import market_indices

    monkeypatch.setattr("shared.config.CACHE_TTL_MINUTES", 0)
    monkeypatch.setattr(
        market_indices,
        "_fetch_live_market_summary_uncached",
        lambda: {"nifty": {"price": 0, "change": 0, "change_pct": 0, "is_live": False}},
    )
    assert market_indices.fetch_live_market_summary()["nifty"]["is_live"] is False

    monkeypatch.setattr(market_indices, "_fetch_yahoo_series", lambda t, d: pd.Series(dtype=float))
    out = market_indices.fetch_benchmark_series("^CNXSC", 365)
    assert out.empty

    eq = pd.Series([100.0], index=pd.to_datetime(["2024-01-01"]))
    debt = pd.Series([200.0], index=pd.to_datetime(["2024-06-01"]))
    monkeypatch.setattr(market_indices, "_fetch_yahoo_series", lambda t, d: eq if "NSEI" in t else debt)
    assert market_indices._build_hybrid_benchmark(365).empty
