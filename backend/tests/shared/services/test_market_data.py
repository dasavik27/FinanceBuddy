...
def test_market_data_amfi_error_and_ter_branches(monkeypatch):
    from shared.services import market_data
    from shared.cache import MarketCache

    market_data.clear_market_data_cache()
    MarketCache.invalidate_all()
    monkeypatch.setattr(market_data.urllib.request, "urlopen", MagicMock(side_effect=RuntimeError("net")))
    live, isin, dt = market_data._fetch_amfi_data_uncached()
    assert live == {}

    ter_csv = "Scheme Code,Scheme Name,TER Date,TER_DIRECT\n119551,Fund,05-Aug-2026,1.05\n"
    mock_resp = MagicMock()
    mock_resp.read.return_value = ter_csv.encode()
    mock_resp.__enter__.return_value = mock_resp
    monkeypatch.setattr(market_data.urllib.request, "urlopen", lambda req, timeout=15: mock_resp)
    assert isinstance(market_data._fetch_amfi_ter_all_uncached(), dict)

def test_market_data_search_empty_and_peer_errors(monkeypatch):
    from shared.services import market_data

    assert market_data.search_mutual_funds("") == []
    monkeypatch.setattr(market_data, "_search_mutual_funds_uncached", lambda q: [{"name": "X"}])
    assert market_data.search_mutual_funds("hdfc")
...
def test_market_data_nav_series_corrupt_cache(monkeypatch):
    ...
    MarketCache.set("nav_series_119551", {"bad-date": "not-a-float"})
    monkeypatch.setattr(
        market_data, "get_provider",
        lambda: MagicMock(fetch_nav_series=lambda code, days: pd.Series(
            [1.0, 2.0], index=pd.date_range("2024-01-01", periods=2),
        )),
    )
    s = market_data._load_full_nav_series("119551", refresh=False)
