...
def test_budget_derived_frame_fingerprint_none(monkeypatch):
    assert derived.frame_fingerprint(None) == "none"
    ...
    assert derived.frame_fingerprint(pd.DataFrame({"bad": [1]})) is None
    key = derived.frame_fingerprint(pd.DataFrame([{"a": 1}]))
    assert key and key.startswith("1x1:")
    assert isinstance(derived.stats(), dict)
