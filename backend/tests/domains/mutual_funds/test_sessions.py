...
def test_mf_sessions_categorize_columns():
    from domains.mutual_funds import sessions as mf_sessions

    df = pd.DataFrame({"Fund": ["A"] * 20, "Category": ["Large Cap"] * 20})
    out = mf_sessions._compact_dtypes(df)
    assert str(out["Category"].dtype) == "category"

    df2 = pd.DataFrame({"Fund": [object()] * 20})
    out2 = mf_sessions._compact_dtypes(df2)
    assert len(out2) == 20
